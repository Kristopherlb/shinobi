...
export const createCatalogCommand = () => {
    ...
    command
        ...
        try {
            ...
            entries
                ...
                const requiredCapabilities = typeof creator.getRequiredCapabilities === 'function'
                    ? creator.getRequiredCapabilities() : [];
                if (Array.isArray(requiredCapabilities) && requiredCapabilities.length > 0) {
                    lines.push('   Requires:\n' + requiredCapabilities.map((cap) => `     • ${cap}`).join('\n'));
                }
