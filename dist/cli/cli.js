...
program
    ...
    .option('--target <target>', 'Deployment target (aws, localstack)', 'aws')
    .action(async (options, cmd) => {
    try {
        const dependencies = cmd.dependencies;
...
program
    .command('up')
    .description('Deploy the service infrastructure')
    .option('--file, -f <file>', 'Path to service.yml file')
    .option('--env <env>', 'Target environment', 'dev')
    .option('--target <target>', 'Deployment target (aws, localstack)', 'aws')
    .action(async (options, cmd) => {
    try {
        const dependencies = cmd.dependencies;
        const deployCommand = compositionRoot.createDeployCommand(dependencies);
        await deployCommand.execute(options);
        process.exit(0);
    }
    catch (error) {
        const dependencies = cmd.dependencies;
        dependencies.logger.error('Deployment failed', error);
        process.exit(2);
    }
});
// svc inventory command
(0, inventory_1.registerInventoryCommand)(program);
// svc local command group
(0, local_1.registerLocalCommands)(program);
