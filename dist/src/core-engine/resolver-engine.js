...
const component_factory_provider_1 = require("./component-factory-provider");
const binding_strategies_1 = require("./binding-strategies");
...
class ResolverEngine {
    ...
    async instantiateComponents(validatedConfig, stack) {
        ...
        const factory = component_factory_provider_1.ComponentFactoryProvider.createFactory(complianceFramework);
     ...
     */
    async applyPlatformServices(components, validatedConfig) {
        ...
        if (serviceContext.serviceRegistry.observability?.enabled) {
            enabledServices.push(new observability_service_1.ObservabilityService(serviceContext));
        }
        ...
        for (const service of enabledServices) {
            ...
            for (const component of components) {
                try {
                    ...
                    this.dependencies.logger.warn(`Failed to apply ${service.name} to component ${component.spec.name}: ${error instanceof Error ? error.message : 'Unknown error'}`);
