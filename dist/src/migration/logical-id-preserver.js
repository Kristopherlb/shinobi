...
exports.LogicalIdPreserver = void 0;
/**
 * Generates logical ID mappings to ensure CloudFormation state preservation
 */
class LogicalIdPreserver {
    logger;
    constructor(logger) {
        this.logger = logger;
    }
    async generateLogicalIdMap(analysisResult, mappingResult) {
        ...
        for (const mappedResource of mappingResult.mappedResources) {
            const originalResource = analysisResult.resources.find(r => r.logicalId === mappedResource.logicalId);
            if (!originalResource) {
            ...
            const newLogicalId = this.generateExpectedLogicalId(mappedResource.componentName, mappedResource.componentType, originalResource.type);
            // Determine preservation strategy
            const strategy = this.determinePreservationStrategy(originalResource.logicalId, newLogicalId, originalResource.type);
            const mapping = {
    ...
    generateExpectedLogicalId(componentName, componentType, resourceType) {
    ...
    getResourceSuffix(componentType, resourceType) {
    ...
    determinePreservationStrategy(originalId, expectedNewId, resourceType) {
    ...
    detectLogicalIdConflicts(mappings) {
        ...
        for (const [newId, originalIds] of newIdCounts.entries()) {
            if (originalIds.length > 1) {
                conflicts.push(`Conflict detected: Multiple resources would map to logical ID '${newId}': ${originalIds.join(', ')}`);
            }
        }
        return conflicts;
    }
     ...
     */
    generateCdkAspectCode(preservationResult) {
        const aspectCode = `
/**
...
`;
     ...
     */
    generatePreservationInstructions(preservationResult) {
        const instructions = [];
     ...
     */
    validatePreservation(preservationResult, originalTemplate, newTemplate) {
        const issues = [];
        ...
        for (const mapping of preservationResult.mappings) {
            ...
            if (originalResource && newResource && originalResource.Type !== newResource.Type) {
                issues.push(`Resource type mismatch for ${mapping.originalId}: ${originalResource.Type} -> ${newResource.Type}`);
            }
        }
        ...
        for (const originalId of originalResourceIds) {
            if (!mappedOriginalIds.has(originalId)) {
                ...
                if (this.isStatefulResourceType(resourceType)) {
                    issues.push(`Stateful resource ${originalId} (${resourceType}) was not mapped and may be deleted during migration`);
                }
            }
        }
        return {
    ...
    isStatefulResourceType(resourceType) {
        const statefulTypes = [
            'AWS::RDS::DBInstance',
            'AWS::S3::Bucket',
            'AWS::DynamoDB::Table',
...
exports.LogicalIdPreserver = LogicalIdPreserver;
