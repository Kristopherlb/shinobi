...
describe("agent", () => {
  it("planner requires analyze_repo tool", async () => {
  ...
  it("reviewer parses structured notes", async () => {
    const llmClient = createLlmClient({
      provider: "mock",
    ...
    expect(notes.some((note) => note.includes("README"))).toBe(true);
  });

  it("planner retries on invalid JSON and repairs output", async () => {
    let attempts = 0;
    const llmClient = createLlmClient({
      provider: "mock",
      responder: () => {
