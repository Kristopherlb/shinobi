...
export class SynthCommand {
  ...
  async execute(options: SynthOptions): Promise<SynthResult> {
    ...
    try {
      ...
      // Resolve output directory relative to manifest file's directory, not current working directory
      // This ensures cdk.out is created next to the service.yml, not in apps/svc
      const manifestDir = path.dirname(manifestPath);
      const outputDir = options.output 
        ? (path.isAbsolute(options.output) ? options.output : path.resolve(manifestDir, options.output))
        : path.resolve(manifestDir, 'cdk.out');
