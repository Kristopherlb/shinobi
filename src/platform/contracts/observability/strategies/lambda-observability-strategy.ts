...
export class LambdaObservabilityStrategy {
  ...
  async instrumentLambda(context: LambdaObservabilityContext): Promise<ObservabilityBindingResult> {
    ...
    const complianceActions = this.createComplianceActions(context);

    return {
      environmentVariables: Object.freeze(environmentVariables),
      iamPolicies: Object.freeze(iamPolicies),
      cloudWatchLogGroups: Object.freeze([{
      ...
      xrayConfigurations: Object.freeze(xrayConfigurations),
      adotConfigurations: Object.freeze(adotConfigurations || []),
      sidecarConfigurations: Object.freeze([]), // Not applicable for Lambda
      agentConfigurations: Object.freeze([]), // Not applicable for Lambda
  ...
  private createLogGroup(lambdaFunction: Function, componentName: string, environment: string): LogGroup {
    ...
    return new LogGroup(lambdaFunction, 'ObservabilityLogGroup', {
      logGroupName,
      retention: this.mapRetentionDays(this.config.logging.retentionDays),
      encryptionKey: this.config.security.encryptionAtRest ? undefined : undefined, // TODO: Add KMS key
      removalPolicy: this.config.tier === 'fedramp-high' ? 
        RemovalPolicy.RETAIN : RemovalPolicy.DESTROY // Retain logs for high compliance
    });
  }
  ...
  private mapRetentionDays(days: number): RetentionDays {
    ...
    if (days <= 1460) return RetentionDays.FOUR_YEARS;
    if (days <= 1825) return RetentionDays.FIVE_YEARS;
    if (days <= 2190) return RetentionDays.SIX_YEARS;
    if (days <= 2555) return RetentionDays.SEVEN_YEARS;
    if (days <= 2920) return RetentionDays.EIGHT_YEARS;
    if (days <= 3285) return RetentionDays.NINE_YEARS;
