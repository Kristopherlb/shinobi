...
export class EcsLoggingHandler implements ILoggingHandler {
  ...
  /**
   * Determine security classification for ECS logs
   */
  private determineSecurityClassification(context: PlatformServiceContext): 'public' | 'internal' | 'confidential' | 'cui' | 'phi' {
    switch (context.complianceFramework) {
      case 'fedramp-high':
        return 'cui'; // Container logs may contain CUI
      case 'fedramp-moderate':
        return 'confidential'; // Application logs are confidential
      default:
        return 'internal'; // Internal application logs
    }
  }
