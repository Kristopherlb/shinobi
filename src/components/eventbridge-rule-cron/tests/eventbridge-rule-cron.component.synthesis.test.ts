/**
 * EventBridgeRuleCronComponent Component Synthesis Test Suite
...
const synthesizeComponent = (
  ...
  spec: ComponentSpec
): { component: EventBridgeRuleCronComponentComponent; template: Template } => {
  const app = new App();
  const stack = new Stack(app, 'TestStack');
  
  const component = new EventBridgeRuleCronComponentComponent(stack, spec, context);
  component.synth();
...
describe('EventBridgeRuleCronComponentComponent Synthesis', () => {
  
  describe('Default Happy Path Synthesis', () => {
    
    it('should synthesize basic eventbridge-rule-cron with commercial compliance', () => {
      ...
      expect(component.getType()).toBe('eventbridge-rule-cron');
    });
