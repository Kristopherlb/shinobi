...
const synthesizeComponent = (
  ...
  spec: ComponentSpec
): { component: SageMakerNotebookInstanceComponentComponent; template: Template } => {
  const app = new App();
  const stack = new Stack(app, 'TestStack');
  
  const component = new SageMakerNotebookInstanceComponentComponent(stack, spec, context);
  component.synth();
...
describe('SageMakerNotebookInstanceComponentComponent Synthesis', () => {
  
  describe('Default Happy Path Synthesis', () => {
    
    it('should synthesize basic sagemaker-notebook-instance with commercial compliance', () => {
      ...
      expect(component.getType()).toBe('sagemaker-notebook-instance');
    });
