/**
 * SsmParameterComponent Component Synthesis Test Suite
...
import { SsmParameterConfig } from '../ssm-parameter.builder';
...
const createMockSpec = (config: Partial<SsmParameterConfig> = {}): ComponentSpec => ({
...
const synthesizeComponent = (
  ...
  spec: ComponentSpec
): { component: SsmParameterComponentComponent; template: Template } => {
  const app = new App();
  const stack = new Stack(app, 'TestStack');
  
  const component = new SsmParameterComponentComponent(stack, spec, context);
  component.synth();
...
describe('SsmParameterComponentComponent Synthesis', () => {
  
  describe('Default Happy Path Synthesis', () => {
    
    it('should synthesize basic ssm-parameter with commercial compliance', () => {
