 ...
 * Handles compliance:config-rule bindings with mandatory compliance enforcement
 */
...
export class ConfigBinderStrategy extends UnifiedBinderStrategyBase {
    ...
    return [
      {
        ...
        description: 'TODO: Add description for compliance:config-rule binding',
  ...
  protected async doBind(context: BindingContext): Promise<Omit<EnhancedBindingResult, 'compliance'>> {
    ...
    return await this.bindToConfig-rule(context, targetCapabilityData);
  }
   ...
   */
  private async bindToConfig-rule(
    ...
    //       new PolicyStatement({
    //         effect: Effect.ALLOW,
    //         actions: ['compliance:config-rule:Get*', 'compliance:config-rule:Describe*'],
