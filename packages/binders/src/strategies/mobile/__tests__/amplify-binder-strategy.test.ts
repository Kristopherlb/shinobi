...
describe('AmplifyBinderStrategy', () => {
  ...
  describe('AmplifyBind__CustomActionsOverride__ReplacesCoarseActions', () => {
    const metadata = {
      id: 'TP-binders-amplify-012',
      level: 'unit' as const,
      capability: 'Custom actions override replaces default Amplify app actions',
      oracle: 'exact' as const,
      determinism: 'deterministic' as const,
      naming: { pattern: 'AmplifyBind__Condition__Outcome', example: 'AmplifyBind__CustomActionsOverride__ReplacesCoarseActions' },
      invariants: [
        'IAM policy actions match provided custom actions array',
        'Default Amplify app actions are not included when custom actions provided',
        'Actions array is used directly (replaces coarse access)'
      ],
      fixtures: ['MockSourceComponent', 'MockTargetComponent'],
      inputs: {
        shape: 'BindingContext with amplify:app capability and directive.actions array',
    ...
    test('AmplifyBind__CustomActionsOverride__ReplacesCoarseActions', async () => {
      const strategy = new AmplifyBinderStrategy();
      const customActions = ['amplify:GetApp', 'amplify:ListApps'];
      const appArn = 'arn:aws:amplify:us-east-1:123456789012:apps/test-app';
      const target = createMockTargetComponent('amplify-app', {
        'amplify:app': {
          appArn,
