...
describe('ElasticBeanstalkBinderStrategy', () => {
  ...
  describe('ElasticBeanstalkBind__CustomActionsOverride__ReplacesCoarseActions', () => {
    const metadata = {
      id: 'TP-binders-compute-elasticbeanstalk-010',
      level: 'unit' as const,
      capability: 'Custom actions override replaces default Elastic Beanstalk actions',
      oracle: 'exact' as const,
      determinism: 'deterministic' as const,
      naming: { pattern: 'ElasticBeanstalkBind__Condition__Outcome', example: 'ElasticBeanstalkBind__CustomActionsOverride__ReplacesCoarseActions' },
      invariants: [
        'IAM policy actions match provided custom actions array',
        'Default Elastic Beanstalk actions are not included when custom actions provided',
        'Actions array is used directly (replaces coarse access)'
      ],
      fixtures: ['MockSourceComponent', 'MockTargetComponent'],
      inputs: {
        shape: 'BindingContext with elasticbeanstalk:application capability and directive.actions array',
    ...
    test('ElasticBeanstalkBind__CustomActionsOverride__ReplacesCoarseActions', async () => {
      const strategy = new ElasticBeanstalkBinderStrategy();
      const customActions = ['elasticbeanstalk:DescribeApplications', 'elasticbeanstalk:ListApplications'];
      const target = createMockTargetComponent('elasticbeanstalk-application', {
        'elasticbeanstalk:application': {
          type: 'elasticbeanstalk:application',
          applicationArn: 'arn:aws:elasticbeanstalk:us-east-1:123456789012:application/test-app',
      ...
      const policy = result.iamPolicies.find(p => p.description.includes('application'));
