...
export class Logger {
  ...
  private writeLogEvent(logEvent: LogEvent): void {
    ...
    // Detect if running in MCP stdio mode (Cursor local)
    // In stdio mode, stdout is reserved for JSON-RPC, so use stderr for logs
    // In server/docker mode, use stdout (standard for containerized apps)
    const isMcpStdioMode = process.env.MCP_TRANSPORT === 'stdio' || 
                            (process.stdin.isTTY === false && process.env.NODE_ENV !== 'production');
    
    if (isMcpStdioMode) {
      // MCP stdio mode: write to stderr to avoid polluting JSON-RPC on stdout
      process.stderr.write(jsonLog);
    } else {
      // Server/Docker mode: write to stdout (Docker standard)
