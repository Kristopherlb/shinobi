/**
 * CognitoUserPoolComponent Triad Matrix Tests
 * 
...
import { CognitoUserPoolComponentConfigBuilder } from '../src/cognito-user-pool.builder.js';
...
describe('CognitoUserPoolComponent - Triad Matrix Tests', () => {
  beforeEach(() => {
    platformConfigSpy = vi
      .spyOn(CognitoUserPoolComponentConfigBuilder.prototype as any, '_loadPlatformConfiguration')
  ...
  frameworks.forEach((framework) => {
    describe(`${framework} Framework`, () => {
      ...
      beforeEach(() => {
        ...
        spec = {
          name: `test-pool-${framework}`,
          type: 'cognito-user-pool',
          config: {
            poolName: `test-pool-${framework}`,
            ...(isHighRisk ? { highRiskEnvironment: true } : {})
          }
        };
      });

      it(`Synthesis__${framework}__CreatesUserPoolWithFrameworkDefaults`, () => {
        const component = new CognitoUserPoolComponent(stack, `TestPool-${framework}`, context, spec);
        component.synth();

        const template = Template.fromStack(stack);
        template.hasResourceProperties('AWS::Cognito::UserPool', Match.anyValue());
      ...
      it(`Tagging__${framework}__AppliesFrameworkSpecificTags`, () => {
        ...
        template.hasResourceProperties('AWS::Cognito::UserPool', {
          UserPoolTags: Match.objectLike({
            'service-name': TEST_CONTEXTS[framework].serviceName,
            'compliance-framework': TEST_CONTEXTS[framework].complianceFramework
          })
        });
      });
    });
