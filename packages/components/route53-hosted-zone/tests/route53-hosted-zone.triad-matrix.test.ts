/**
 * Route53HostedZoneComponent Triad Matrix Tests
 * 
...
import { Route53HostedZoneComponentConfigBuilder } from '../route53-hosted-zone.builder.js';
...
describe('Route53HostedZoneComponent - Triad Matrix Tests', () => {
  beforeEach(() => {
  ...
  frameworks.forEach((framework) => {
    describe(`${framework} Framework`, () => {
      ...
      it(`Synthesis__${framework}__CreatesZoneWithFrameworkDefaults`, () => {
        const component = new Route53HostedZoneComponent(stack, `TestZone-${framework}`, context, spec);
        component.synth();

        const template = Template.fromStack(stack);

        // All frameworks should create Route53 hosted zone
        // Zone names are lowercased by the builder, so convert framework name to lowercase
        const expectedZoneName = `example-${framework.toLowerCase()}.com.`;
      ...
      it(`Tagging__${framework}__AppliesFrameworkSpecificTags`, () => {
        ...
        // Check for tags individually without assuming order
        // Match.arrayWith() checks for presence but may be sensitive to order, so check each tag separately
        template.hasResourceProperties('AWS::Route53::HostedZone', {
