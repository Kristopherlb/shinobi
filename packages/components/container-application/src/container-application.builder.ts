...
export class ContainerApplicationComponentConfigBuilder extends ConfigBuilder<ContainerApplicationConfig> {
  ...
  protected getComplianceFrameworkDefaults(): Partial<ContainerApplicationConfig> {
    ...
    const componentConfig = this.builderContext.spec.config as Partial<ContainerApplicationConfig> | undefined;
    ...
    if (isHighRisk) {
      ...
      return {
        security: {
          enableEncryption: true,
          enableVpcFlowLogs: true,
          enableWaf: true // WAF required for high-risk environments
        },
        observability: {
          enabled: true,
          logRetentionDays: 1095, // 3 years (can be overridden to 2555 for higher risk)
          cpuThreshold: 80,
          memoryThreshold: 85,
          enableTracing: true,
          enableMetrics: true
