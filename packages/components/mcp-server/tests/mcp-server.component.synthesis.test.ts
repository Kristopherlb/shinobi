/**
 * McpServerComponent Component Synthesis Test Suite
...
const synthesizeComponent = (
  ...
  spec: ComponentSpec
): { component: McpServerComponentComponent; template: Template } => {
  const app = new App();
  const stack = new Stack(app, 'TestStack');
  
  const component = new McpServerComponentComponent(stack, spec, context);
  component.synth();
...
describe('McpServerComponentComponent Synthesis', () => {
  
  describe('Default Happy Path Synthesis', () => {
    
    it('should synthesize basic mcp-server with commercial compliance', () => {
      ...
      expect(component.getType()).toBe('mcp-server');
    });
