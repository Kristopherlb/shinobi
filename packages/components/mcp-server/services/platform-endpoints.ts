 ...
 */
export class PlatformEndpointsService {
  /**
   * GET /platform/components
   * Lists all available, versioned components.
   */
  async getComponents(): Promise<PlatformComponent[]> {
   ...
   */
  async getComponentSchema(componentType: string): Promise<ComponentSchema> {
   ...
   */
  async getCapabilities(): Promise<CapabilityDefinition[]> {
    return [
      {
        name: 'api:rest',
   ...
   */
  async getBindings(): Promise<BindingMatrix[]> {
    return [
      {
  ...
  async validateManifest(manifest: any): Promise<ValidationResult> {
    ...
    if (manifest.components) {
      manifest.components.forEach((component: any, index: number) => {
        if (component.binds && Array.isArray(component.binds)) {
          component.binds.forEach((binding: any, bindingIndex: number) => {
            if (!binding.to) {
              errors.push({
    ...
    if (manifest.components && manifest.components.length > 0) {
      const hasDatabase = manifest.components.some((c: any) => c.type === 'rds-postgres');
      const hasApi = manifest.components.some((c: any) => c.type === 'lambda-api');
