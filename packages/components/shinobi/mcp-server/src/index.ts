...
class ShinobiMcpServer {
  ...
  private async handleGenerateComponent(args: any) {
    const {
      ...
      promptDocumentPath = 'docs/prompts/agent-component-builder.md'
    } = args;
    ...
    const componentGeneration = {
      ...
      progress: {
        ...
        stageDetails: stages.map((stage: number) => ({
          stage,
  ...
  private async handleScaffoldComponent(args: any) {
  ...
  private async handleUpgradeComponent(args: any) {
  ...
  private async handleListFeatureFlags(args: any) {
    ...
    if (category !== 'all') {
      ...
      filteredFlags = Object.fromEntries(filteredEntries) as typeof SHINOBI_FEATURE_FLAGS;
    }
  ...
  private async handleGetFeatureFlag(args: any) {
    const { flagKey, environment = 'development', context = {} } = args;

    try {
      // Mock feature flags data (same as above)
      const SHINOBI_FEATURE_FLAGS: Record<string, any> = {
        'shinobi.run-audited-tests-only': {
          flagKey: 'shinobi.run-audited-tests-only',
          flagType: 'boolean',
          defaultValue: false,
  ...
  private async handleSetFeatureFlag(args: any) {
  ...
  private async handleToggleFeatureFlag(args: any) {
  ...
  private async handleEvaluateFeatureFlags(args: any) {
    ...
    try {
      ...
      const results = flagKeys.map((flagKey: string) => {
        const config = SHINOBI_FEATURE_FLAGS[flagKey];
        if (!config) {
          return {
            flagKey,
        ...
        content: [
          {
            ...
            text: JSON.stringify({
              ...
              notFound: results.filter((r: any) => r.error).length
            }, null, 2)
          }
        ]
      };
   ...
   */
  private evaluateFeatureFlag(config: any, environment: string, context: any): any {
    ...
    if (config.targetingRules) {
      ...
      // Check conditions
      if (rules.conditions) {
        const conditionsMet = rules.conditions.every((condition: any) => {
  ...
  private parsePromptDocument(promptPath: string): any {
  ...
  private validateArchitecturalPatterns(componentType: string, promptRules: any): any {
    return {
      baseComponentInheritance: true,
      configBuilderPattern: true,
      sixStepSynthPattern: true,
  ...
  private validateConfigBuilderPatterns(componentType: string, promptRules: any): any {
    return {
  ...
  private checkEnvironmentLogicSeparation(componentType: string): any {
    return {
  ...
  private validatePromptDocumentCompliance(componentType: string, stages: number[], promptRules: any): any {
    return {
  ...
  private generateConfigurationSurface(componentType: string): string[] {
    return [
      'instanceType',
      'vpcId',
      'subnetIds',
  ...
  private generateCapabilitiesList(componentType: string): string[] {
    return [
      'compute:ec2',
  ...
  private generateEnvironmentAssumptions(componentType: string): string[] {
    return [
  ...
  private generateSecurityFeatures(componentType: string, framework: string): string[] {
  ...
  private generateComplianceControls(componentType: string, framework: string): string[] {
    return [
  ...
  private generateOSCALMapping(componentType: string, framework: string): any {
    return {
      componentDefinition: {
  ...
  private generateTestResults(componentType: string): any {
    return {
      totalTests: 24,
      passed: 24,
      failed: 0,
  ...
  private generateCoverageReport(componentType: string): any {
    return {
  ...
  private getStageDuration(stage: number): string {
    const durations = {
      0: '30-60s',
      1: '15-30s',
      2: '45-90s',
    ...
    return durations[stage as keyof typeof durations] || 'unknown';
  }

  private getStageArtifactCount(stage: number): number {
    ...
    return artifactCounts[stage as keyof typeof artifactCounts] || 0;
  }

  // New Handler Methods

  private async handleValidateComponentPatterns(args: any) {
    const {
      componentPath,
      validationLevel = 'strict',
      checkConfigBuilder = true,
      checkBaseComponent = true,
      checkEnvironmentLogic = true
    } = args;
  ...
  private async handleRunComponentTests(args: any) {
    const {
      componentPath,
      testTypes = ['unit', 'compliance'],
  ...
  private async handleAuditComponentCompliance(args: any) {
    const {
      componentPath,
      complianceFramework = 'commercial',
      includeOSCAL = true,
      includeRegoPolicies = true,
  ...
  private async handleComponentWizard(args: any) {
