...
import { EcrRepositoryConfig } from '../ecr-repository.builder';
...
const synthesizeComponent = (
  ...
  spec: ComponentSpec
): { component: EcrRepositoryComponentComponent; template: Template } => {
  const app = new App();
  const stack = new Stack(app, 'TestStack');
  
  const component = new EcrRepositoryComponentComponent(stack, spec, context);
  component.synth();
...
describe('EcrRepositoryComponentComponent Synthesis', () => {
