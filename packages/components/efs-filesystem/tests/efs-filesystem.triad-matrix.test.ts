/**
 * EfsFilesystemComponent Triad Matrix Tests
 * 
...
import { EfsFilesystemComponentConfigBuilder } from '../src/efs-filesystem.builder.js';
...
describe('EfsFilesystemComponent - Triad Matrix Tests', () => {
  beforeEach(() => {
    platformConfigSpy = vi
      .spyOn(EfsFilesystemComponentConfigBuilder.prototype as any, '_loadPlatformConfiguration')
  ...
  frameworks.forEach((framework) => {
    describe(`${framework} Framework`, () => {
      ...
      beforeEach(() => {
        ...
        spec = {
          name: `test-efs-${framework}`,
          type: 'efs-filesystem',
          config: {
            ...(isHighRisk ? { highRiskEnvironment: true } : {}),
            // Enable VPC configuration since we're providing a VPC via context
            vpc: {
              enabled: true
            }
          }
      ...
      it(`Synthesis__${framework}__CreatesFilesystemWithFrameworkDefaults`, () => {
        const component = new EfsFilesystemComponent(stack, `TestEfs-${framework}`, context, spec);
        component.synth();

        const template = Template.fromStack(stack);
        template.hasResourceProperties('AWS::EFS::FileSystem', Match.anyValue());
        expect(component.getType()).toBe('efs-filesystem');
      });
