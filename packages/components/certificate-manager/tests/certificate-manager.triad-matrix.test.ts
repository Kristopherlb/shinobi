/**
 * CertificateManagerComponent Triad Matrix Tests
 * 
...
import { CertificateManagerComponentConfigBuilder } from '../src/certificate-manager.builder.js';
...
describe('CertificateManagerComponent - Triad Matrix Tests', () => {
  beforeEach(() => {
    platformConfigSpy = vi
      .spyOn(CertificateManagerComponentConfigBuilder.prototype as any, '_loadPlatformConfiguration')
  ...
  frameworks.forEach((framework) => {
    describe(`${framework} Framework`, () => {
      ...
      beforeEach(() => {
        ...
        hostedZone = new route53.PublicHostedZone(stack, `TestZone-${framework}`, {
        ...
        spec = {
          name: `test-cert-${framework}`,
          type: 'certificate-manager',
          config: {
            domainName: `example-${framework}.com`,
            validation: {
              method: 'DNS',
      ...
      it(`Synthesis__${framework}__CreatesCertificateWithFrameworkDefaults`, () => {
        const component = new CertificateManagerComponent(stack, `TestCert-${framework}`, context, spec);
        component.synth();

        const template = Template.fromStack(stack);
        template.hasResourceProperties('AWS::CertificateManager::Certificate', Match.anyValue());
