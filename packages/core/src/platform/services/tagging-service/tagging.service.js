...
export class TaggingService {
    ...
    buildStandardTags(context) {
        ...
        const complianceFrameworkValue = this.resolveString(overrides['compliance-framework'], labels['compliance-framework'], labels['complianceFramework'], tags['compliance-framework'], tags['complianceFramework'], context.complianceFramework, 'none');
        const dataClassification = governance?.dataClassification
            ?? this.resolveString(overrides['data-classification'], labels['data-classification'], labels['dataClassification'], tags['data-classification'], tags['dataClassification'], 'internal');
        const backupRequired = this.resolveBooleanString(overrides['backup-required'], governance ? (governance.backupRequired ? 'true' : 'false') : undefined, tags['backup-required'], true);
        const monitoringLevel = this.resolveString(overrides['monitoring-level'], governance?.monitoringLevel, labels['monitoring-level'], labels['monitoringLevel'], tags['monitoring-level'], tags['monitoringLevel'], 'standard');
        const retentionPeriod = this.resolveString(overrides['retention-period'], governance ? `${governance.logRetentionDays}-days` : undefined, labels['retention-period'], labels['retentionPeriod'], tags['retention-period'], tags['retentionPeriod'], '1-year');
        const costCenter = this.resolveString(overrides['cost-center'], labels['cost-center'], labels['costCenter'], tags['cost-center'], tags['costCenter'], 'platform');
        const owner = this.resolveString(overrides['owner'], labels['owner'], tags['owner'], 'platform-engineering');
        const project = this.resolveString(overrides['project'], labels['project'], tags['project'], context.serviceName);
        return {
            // Core Service Tags
