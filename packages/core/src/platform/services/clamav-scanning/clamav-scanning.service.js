...
export class ClamAvScanningServiceImpl {
    scope;
    context;
    scanningResources = [];
    quarantineResources = [];
    ...
    createClamAvFunction(config, storageType = 's3') {
        ...
        const clamavFunction = new lambda.Function(this.scope, functionName, {
            ...
            layers: [
                ...
                lambda.LayerVersion.fromLayerVersionArn(this.scope, 'ClamAvLayer', 'arn:aws:lambda:us-east-1:123456789012:layer:clamav:1')
            ]
        });
...
`;
    ...
    addClamAvPermissions(lambdaFunction, storageType) {
        const permissions = [];
        // S3 permissions
        if (storageType === 's3') {
    ...
    createQuarantineBucket(sourceBucket) {
    ...
    configureS3UploadScanning(bucket, clamavFunction, quarantineBucket) {
        ...
        bucket.addEventNotification(s3.EventType.OBJECT_CREATED, new s3_notifications.LambdaDestination(clamavFunction));
    }
    configureScheduledScanning(clamavFunction, bucket) {
    ...
    configureEFSAccess(clamavFunction, efsArn) {
        // Configure EFS access for Lambda
        // Implementation would depend on EFS setup
    }
    configureScheduledEFSScanning(clamavFunction, efsArn) {
    ...
    configureFSxAccess(clamavFunction, fsxArn) {
        // Configure FSx access for Lambda
        // Implementation would depend on FSx setup
    }
    configureScheduledFSxScanning(clamavFunction, fsxArn) {
 ...
 */
export function createClamAvScanningService(scope, context) {
    return new ClamAvScanningServiceImpl(scope, context);
}
//# sourceMappingURL=clamav-scanning.service.js.map