...
import { ArtifactSerializerFactory } from './artifact-serializer';

export class StandardArtifactWriter implements ArtifactWriter {
  private serializer = ArtifactSerializerFactory.create('json');

  async writePlanArtifact(artifact: PlanArtifact, outputDir: string): Promise<string[]> {
  ...
  async writeDeploymentArtifact(artifact: DeploymentArtifact, outputDir: string): Promise<string[]> {
    const writtenFiles: string[] = [];
  ...
  private generateMigrationReportMarkdown(report: MigrationReport): string {
