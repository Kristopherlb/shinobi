...
export class CompliancePlanGenerator {
     ...
     */
    async generateCompliancePlan(componentId, componentType, framework, componentConfig, config) {
        // Generate the compliance plan
        const plan = this.controlMappingService.generateCompliancePlan(componentId, componentType, framework, componentConfig);
        // Add tagging policy if requested
        if (config.includeTaggingPolicy) {
            const taggingPolicy = this.taggingService.generateTaggingPolicy(componentType, {
            ...
            plan.taggingPolicy = taggingPolicy;
        }
     ...
     */
    async persistCompliancePlan(componentId, plan, config) {
     ...
     */
    async generateComplianceSummary(components, config) {
        const summary = {
        ...
        for (const component of components) {
            ...
            const plan = await this.generateCompliancePlan(component.id, component.type, component.framework, component.config, config);
     ...
     */
    validateManifestCompliance(manifest, framework) {
        const errors = [];
        const warnings = [];
        const missingDataClassifications = [];
...
`;
     ...
     */
    async generateComplianceReport(components, config) {
...
//# sourceMappingURL=compliance-plan-generator.js.map