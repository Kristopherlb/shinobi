 ...
 */
import { ComplianceError } from './platform-binding-trigger-spec.js';
import { loadComplianceRules, getRequiredRules, isRuleRequired, getRuleConfig } from './compliance/rules.js';
import { validateResourcesForStatements, extractServicePrefix } from '../binders/resource-validator.js';
/**
 ...
 */
export class UnifiedBinderStrategyBase {
    /**
    ...
    async bind(context) {
        ...
        const complianceStatus = this.evaluateCompliance(framework, context, bindingResult, rulesOverride, customConfigPath);
     ...
     */
    resolveComplianceFramework(context) {
        if (context.complianceFramework) {
            return context.complianceFramework;
        }
     ...
     */
    getRulesOverride(context) {
        const options = context.directive?.options || {};
        const override = options.complianceRulesOverride;
        if (!override) {
            return undefined;
        }
     ...
     */
    getCustomConfigPath(context) {
        const options = context.directive?.options || {};
        return options.complianceConfigPath;
    }
    /**
     ...
     */
    loadComplianceRules(framework, context) {
        const rulesOverride = this.getRulesOverride(context);
        const customConfigPath = this.getCustomConfigPath(context);
     ...
     */
    isRuleRequired(framework, ruleName, category, context) {
        const rulesOverride = this.getRulesOverride(context);
        const customConfigPath = this.getCustomConfigPath(context);
     ...
     */
    getRequiredRules(framework, category, context) {
        const rulesOverride = this.getRulesOverride(context);
        const customConfigPath = this.getCustomConfigPath(context);
     ...
     */
    getRuleConfig(framework, ruleName, context) {
        const rulesOverride = this.getRulesOverride(context);
        const customConfigPath = this.getCustomConfigPath(context);
     ...
     */
    getComplianceCategory(capability) {
     ...
     */
    evaluateCompliance(framework, context, bindingResult, rulesOverride, customConfigPath) {
        ...
        const actionsTaken = bindingResult.iamPolicies
        ...
        for (const ruleName of requiredRuleNames) {
            ...
            const violation = this.evaluateRule(ruleName, ruleConfig, framework, context, bindingResult);
            if (violation) {
     ...
     */
    evaluateRule(ruleName, ruleConfig, framework, context, bindingResult) {
     ...
     */
    evaluateEncryptionAtRest(ruleConfig, framework, context, bindingResult) {
        // Check environment variables for encryption indicators
        const envVars = bindingResult.environmentVariables;
        const hasEncryption = envVars['KMS_KEY_ID'] || envVars['KMS_KEY_ARN'] ||
            ...
            bindingResult.iamPolicies.some(policy => {
                ...
                const actions = Array.isArray(statementJson.Action)
                    ...
                    : [statementJson.Action];
                return actions.some((action) => action.includes('kms:Encrypt') ||
     ...
     */
    evaluateEncryptionInTransit(ruleConfig, framework, context, bindingResult) {
        // Check environment variables for TLS/HTTPS indicators
        const envVars = bindingResult.environmentVariables;
        const hasTls = envVars['REQUIRE_TLS'] === 'true' ||
            envVars['FORCE_HTTPS'] === 'true' ||
            envVars['TLS_ENABLED'] === 'true' ||
            // Check if endpoint URLs use https://
            Object.values(envVars).some(value => typeof value === 'string' && value.startsWith('https://'));
     ...
     */
    evaluateLeastPrivilegeIAM(ruleConfig, framework, context, bindingResult) {
        ...
        for (const policy of bindingResult.iamPolicies) {
            ...
            const resources = Array.isArray(statementJson.Resource)
                ...
                : [statementJson.Resource];
            const hasWildcardResource = resources.some((resource) => resource === '*' || resource.endsWith('/*'));
            // Allow wildcard resources for some AWS service APIs (like s3:ListBucket)
            // but flag for sensitive operations
            const sensitiveActions = actions.some((action) => action.includes(':Put') ||
...
//# sourceMappingURL=unified-binder-strategy-base.js.map