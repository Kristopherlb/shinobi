 ...
 */
export function loadComplianceRules(framework, customConfigPath, rulesOverride) {
    ...
    if (customConfigPath && fs.existsSync(customConfigPath)) {
        try {
            const config = yaml.load(fs.readFileSync(customConfigPath, 'utf8'));
 ...
 */
export function getRequiredRules(framework, category, customConfigPath, rulesOverride) {
    const rules = loadComplianceRules(framework, customConfigPath, rulesOverride);
    return Object.entries(rules)
        .filter(([_, ruleConfig]) => ruleConfig.categories.includes(category) || ruleConfig.categories.includes('all'))
 ...
 */
export function isRuleRequired(framework, ruleName, category, customConfigPath, rulesOverride) {
    const rules = loadComplianceRules(framework, customConfigPath, rulesOverride);
 ...
 */
export function getRuleConfig(framework, ruleName, customConfigPath, rulesOverride) {
    const rules = loadComplianceRules(framework, customConfigPath, rulesOverride);
...
function validateRulesConfig(rules) {
    ...
    for (const [name, config] of Object.entries(rules)) {
        if (config && typeof config === 'object' && Array.isArray(config.categories)) {
            validated[name] = {
 ...
 */
function getDefaultComplianceRules(framework) {
