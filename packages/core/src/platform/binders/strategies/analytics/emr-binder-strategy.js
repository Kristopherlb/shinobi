 ...
 */
export class EmrBinderStrategy {
    supportedCapabilities = ['emr:cluster', 'emr:step', 'emr:notebook'];
    ...
    async bindToCluster(sourceComponent, targetComponent, binding, context) {
        ...
        if (targetComponent.applications) {
            sourceComponent.addEnvironment('EMR_APPLICATIONS', targetComponent.applications.map((app) => app.name).join(','));
        }
    ...
    async bindToStep(sourceComponent, targetComponent, binding, context) {
        const { access } = binding;
    ...
    async configureSecureClusterAccess(sourceComponent, targetComponent, binding, context) {
...
//# sourceMappingURL=emr-binder-strategy.js.map