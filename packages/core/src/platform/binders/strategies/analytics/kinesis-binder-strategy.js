...
export class KinesisBinderStrategy {
    supportedCapabilities = ['kinesis:stream', 'kinesis:analytics', 'kinesis:firehose'];
    ...
    async bindToAnalytics(sourceComponent, targetComponent, binding, context) {
        const { access } = binding;
        ...
        if (targetComponent.applicationConfiguration) {
            sourceComponent.addEnvironment('KINESIS_ANALYTICS_SQL_APPLICATION', targetComponent.applicationConfiguration.sqlApplicationConfiguration ? 'true' : 'false');
        }
    }
    async bindToFirehose(sourceComponent, targetComponent, binding, context) {
        const { access } = binding;
        ...
        if (targetComponent.processingConfiguration?.processors) {
            targetComponent.processingConfiguration.processors.forEach((processor) => {
                if (processor.type === 'Lambda') {
                    sourceComponent.addToRolePolicy({
                        ...
                        Resource: processor.parameters.find((p) => p.parameterName === 'LambdaArn')?.parameterValue
                    });
                }
            });
        }
    ...
    async configureSecureStreamAccess(sourceComponent, targetComponent, binding, context) {
    ...
    async configureSecureFirehoseAccess(sourceComponent, targetComponent, context) {
        ...
        if (targetComponent.s3DestinationConfiguration?.encryptionConfiguration) {
            ...
            if (targetComponent.s3DestinationConfiguration.encryptionConfiguration.kmsEncryptionConfig) {
                sourceComponent.addEnvironment('KINESIS_FIREHOSE_S3_KMS_KEY_ARN', targetComponent.s3DestinationConfiguration.encryptionConfiguration.kmsEncryptionConfig.awsKMSKeyARN);
                // Grant KMS permissions
                sourceComponent.addToRolePolicy({
                    Effect: 'Allow',
                    Action: [
        ...
        if (targetComponent.s3DestinationConfiguration?.backupConfiguration) {
            ...
            sourceComponent.addEnvironment('KINESIS_FIREHOSE_BACKUP_S3_BUCKET', targetComponent.s3DestinationConfiguration.backupConfiguration.s3BackupConfiguration.bucketARN);
        }
        // Configure data compression
        if (targetComponent.s3DestinationConfiguration?.compressionFormat) {
            sourceComponent.addEnvironment('KINESIS_FIREHOSE_COMPRESSION_FORMAT', targetComponent.s3DestinationConfiguration.compressionFormat);
        }
    }
}
//# sourceMappingURL=kinesis-binder-strategy.js.map