...
export class KmsBinderStrategy {
    supportedCapabilities = ['kms:key', 'kms:alias', 'kms:grant'];
    ...
    async bindToKey(sourceComponent, targetComponent, binding, context) {
    ...
    async bindToAlias(sourceComponent, targetComponent, binding, context) {
        const { access } = binding;
    ...
    async bindToGrant(sourceComponent, targetComponent, binding, context) {
        // Validate required target component properties
        if (!targetComponent?.keyArn) {
    ...
    async configureSecureKeyAccess(sourceComponent, targetComponent, context) {
        ...
        if (targetComponent?.enableKeyRotation === true) {
        ...
        if (targetComponent?.enableFipsEndpoint === true) {
...
//# sourceMappingURL=kms-binder-strategy.js.map