export class CognitoUserPoolBinderStrategy {
    supportedCapabilities = ['auth:user-pool', 'auth:identity-provider'];
    ...
    bindToUserPool(sourceComponent, targetComponent, binding, access) {
    ...
    bindToIdentityProvider(sourceComponent, targetComponent, binding, access) {
    ...
    applyAccessPolicies(sourceComponent, userPoolArn, access) {
    ...
    applyCustomEnvOverrides(sourceComponent, binding) {
    ...
    addEnvironment(sourceComponent, key, value) {
        if (typeof sourceComponent.addEnvironment === 'function') {
            sourceComponent.addEnvironment(key, value);
        }
    }
}
//# sourceMappingURL=cognito-user-pool-binder-strategy.js.map