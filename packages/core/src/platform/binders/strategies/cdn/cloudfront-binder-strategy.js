 ...
 */
export class CloudFrontBinderStrategy {
    supportedCapabilities = ['cloudfront:distribution', 'cloudfront:origin', 'cloudfront:cache-policy'];
    ...
    async bindToDistribution(sourceComponent, targetComponent, binding, context) {
        ...
        if (targetComponent.origins) {
            targetComponent.origins.forEach((origin) => {
    ...
    async bindToOrigin(sourceComponent, targetComponent, binding, context) {
        const { access } = binding;
    ...
    async bindToCachePolicy(sourceComponent, targetComponent, binding, context) {
        const { access } = binding;
    ...
    async configureSecureDistributionAccess(sourceComponent, targetComponent, context) {
        ...
        if (targetComponent.edgeLocationsUsOnly === true || targetComponent.usOnly === true) {
            sourceComponent.addEnvironment('CLOUDFRONT_EDGE_LOCATIONS_US_ONLY', 'true');
        }
    }
}
//# sourceMappingURL=cloudfront-binder-strategy.js.map