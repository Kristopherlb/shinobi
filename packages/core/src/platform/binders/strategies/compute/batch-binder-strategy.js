...
export class BatchBinderStrategy {
    supportedCapabilities = ['batch:job-queue', 'batch:compute-environment', 'batch:job-definition', 'batch:job'];
    ...
    async bindToJobQueue(sourceComponent, targetComponent, binding, context) {
        ...
        if (targetComponent.computeEnvironmentOrder) {
            sourceComponent.addEnvironment('BATCH_COMPUTE_ENVIRONMENTS', targetComponent.computeEnvironmentOrder.map((ce) => ce.computeEnvironment).join(','));
        }
    }
    async bindToComputeEnvironment(sourceComponent, targetComponent, binding, context) {
        const { access } = binding;
    ...
    async bindToJobDefinition(sourceComponent, targetComponent, binding, context) {
        const { access } = binding;
    ...
    async bindToJob(sourceComponent, targetComponent, binding, context) {
        const { access } = binding;
    ...
    async configureSecureJobEnvironment(sourceComponent, targetComponent, binding, context) {
        ...
        if (targetComponent.secrets) {
            sourceComponent.addEnvironment('BATCH_SECRETS_ARN', targetComponent.secrets.map((s) => s.secretArn).join(','));
            ...
            sourceComponent.addToRolePolicy({
                ...
                Resource: targetComponent.secrets.map((s) => s.secretArn)
            });
        }
    }
}
//# sourceMappingURL=batch-binder-strategy.js.map