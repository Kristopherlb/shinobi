...
export class ElasticBeanstalkBinderStrategy {
    supportedCapabilities = ['elasticbeanstalk:application', 'elasticbeanstalk:environment', 'elasticbeanstalk:version'];
    ...
    async bindToApplication(sourceComponent, targetComponent, binding, context) {
    ...
    async bindToEnvironment(sourceComponent, targetComponent, binding, context) {
        const { access } = binding;
        ...
        if (targetComponent.logGroups) {
            sourceComponent.addToRolePolicy({
                ...
                Resource: targetComponent.logGroups.map((lg) => `arn:aws:logs:${context.region}:${context.accountId}:log-group:${lg}:*`)
            });
        }
    ...
    async bindToVersion(sourceComponent, targetComponent, binding, context) {
        const { access } = binding;
    ...
    async configureSecureEnvironment(sourceComponent, targetComponent, binding, context) {
        ...
        if (targetComponent.autoScalingGroups) {
            ...
            sourceComponent.addToRolePolicy({
                ...
                Resource: targetComponent.autoScalingGroups.map((asg) => `arn:aws:autoscaling:${context.region}:${context.accountId}:autoScalingGroup:*:autoScalingGroupName/${asg}`)
            });
        }
...
//# sourceMappingURL=elastic-beanstalk-binder-strategy.js.map