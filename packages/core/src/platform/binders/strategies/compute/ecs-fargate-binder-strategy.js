...
export class EcsFargateBinderStrategy {
    supportedCapabilities = ['ecs:cluster', 'ecs:service', 'ecs:task-definition', 'otel:environment'];
    ...
    bindTelemetryEnvironment(sourceComponent, targetValues, binding) {
    ...
    async bindToTaskDefinition(sourceComponent, targetComponent, binding, context) {
        // Validate required target component properties
        if (!targetComponent?.taskDefinitionArn) {
    ...
    async configureSecureNetworkAccess(sourceComponent, targetComponent, context, binding) {
        // Ensure secure network access when requested (no framework branching)
        if (sourceComponent.securityGroup) {
            sourceComponent.securityGroup.addIngressRule(sourceComponent.securityGroup, { port: 443, protocol: 'tcp' }, 'HTTPS access for ECS API calls');
        }
...
//# sourceMappingURL=ecs-fargate-binder-strategy.js.map