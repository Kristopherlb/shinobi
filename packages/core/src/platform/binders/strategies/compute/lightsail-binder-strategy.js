...
export class LightsailBinderStrategy {
    supportedCapabilities = ['lightsail:instance', 'lightsail:database', 'lightsail:load-balancer', 'lightsail:container-service'];
    ...
    async bindToDatabase(sourceComponent, targetComponent, binding, context) {
        const { access } = binding;
    ...
    async bindToLoadBalancer(sourceComponent, targetComponent, binding, context) {
        const { access } = binding;
        ...
        if (targetComponent.tlsCertificateSummaries) {
            sourceComponent.addEnvironment('LIGHTSAIL_TLS_CERTIFICATES', targetComponent.tlsCertificateSummaries.map((cert) => cert.name).join(','));
        }
    }
    async bindToContainerService(sourceComponent, targetComponent, binding, context) {
        const { access } = binding;
        ...
        if (targetComponent.containerImages) {
            sourceComponent.addToRolePolicy({
                ...
                Resource: targetComponent.containerImages.map((img) => img.ecrRepositoryArn)
            });
        }
        ...
        if (targetComponent.containerImages) {
            sourceComponent.addEnvironment('LIGHTSAIL_CONTAINER_IMAGES', targetComponent.containerImages.map((img) => img.image).join(','));
        }
    }
    async configureSecureInstanceAccess(sourceComponent, targetComponent, context) {
        ...
        if (targetComponent.networking?.ports) {
            const securePorts = targetComponent.networking.ports.filter((port) => port.protocol === 'tcp' && (port.fromPort === 443 || port.fromPort === 22));
    ...
    async configureSecureDatabaseAccess(sourceComponent, targetComponent, context, binding) {
        ...
        if (targetComponent.backupRetentionEnabled) {
            ...
            sourceComponent.addEnvironment('LIGHTSAIL_BACKUP_RETENTION_DAYS', binding?.options?.backupRetentionDays ? String(binding.options.backupRetentionDays) : '7');
        }
...
//# sourceMappingURL=lightsail-binder-strategy.js.map