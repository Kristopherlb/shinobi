...
export class SageMakerBinderStrategy {
    supportedCapabilities = ['sagemaker:notebook', 'sagemaker:model', 'sagemaker:endpoint', 'sagemaker:training-job'];
    ...
    async bindToNotebook(sourceComponent, targetComponent, binding, context) {
    ...
    async bindToModel(sourceComponent, targetComponent, binding, context) {
        const { access } = binding;
    ...
    async bindToEndpoint(sourceComponent, targetComponent, binding, context) {
        const { access } = binding;
    ...
    async bindToTrainingJob(sourceComponent, targetComponent, binding, context) {
        const { access } = binding;
        ...
        if (targetComponent.inputDataConfig) {
            targetComponent.inputDataConfig.forEach((input) => {
                sourceComponent.addToRolePolicy({
                    Effect: 'Allow',
                    Action: [
                        's3:GetObject',
    ...
    async configureSecureNotebookAccess(sourceComponent, targetComponent, context) {
        ...
        if (targetComponent?.disableRootAccess === true) {
...
//# sourceMappingURL=sagemaker-binder-strategy.js.map