...
export class AmplifyBinderStrategy {
    supportedCapabilities = ['amplify:app', 'amplify:branch', 'amplify:domain'];
    ...
    async bindToApp(sourceComponent, targetComponent, binding, context) {
    ...
    async bindToBranch(sourceComponent, targetComponent, binding, context) {
        const { access } = binding;
        ...
        if (targetComponent.environmentVariables) {
            Object.entries(targetComponent.environmentVariables).forEach(([key, value]) => {
                sourceComponent.addEnvironment(`AMPLIFY_BRANCH_ENV_${key}`, value);
            });
        }
    ...
    async bindToDomain(sourceComponent, targetComponent, binding, context) {
        const { access } = binding;
    ...
    async configureSecureAppAccess(sourceComponent, targetComponent, context) {
        ...
        if (targetComponent?.enableVpc === true) {
...
//# sourceMappingURL=amplify-binder-strategy.js.map