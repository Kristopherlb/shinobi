...
import { validateActionsAgainstAllowList, areCustomActionsAllowed } from './action-allow-lists.js';
/**
 ...
 */
function validateServicePrefix(actions, servicePrefix) {
    const prefix = `${servicePrefix}:`;
    const mismatched = actions.filter(action => !action.startsWith(prefix));
    if (mismatched.length > 0) {
        throw new Error(`Actions must match service prefix '${servicePrefix}:'. ` +
 ...
 */
function validateWildcardActions(actions, framework) {
    ...
    if (framework === 'fedramp-moderate' || framework === 'fedramp-high') {
        throw new Error(`Wildcard actions are not allowed in ${framework} framework. ` +
    ...
    if (framework === 'commercial') {
        console.warn(`Warning: Wildcard actions detected in commercial framework: ${wildcardActions.join(', ')}. ` +
 ...
 */
export function resolveActions(directive, context, getActionsForAccess, servicePrefix) {
    ...
    if (typeof directive.actions === 'string') {
        ...
        if (!profileActions) {
            throw new Error(`Action profile '${directive.actions}' not found in ${context.complianceFramework} framework. ` +
        ...
        const framework = context.complianceFramework;
        ...
        if (!areCustomActionsAllowed(framework)) {
            ...
            const targetName = context.target && typeof context.target.getName === 'function'
                ...
                : 'unknown';
            console.error(`[COMPLIANCE-AUDIT] Custom actions rejected in ${framework} framework. ` +
                `Only action profiles are allowed. ` +
                `Source: ${sourceName}, ` +
                `Target: ${targetName}, ` +
                `Capability: ${directive.capability || 'unknown'}, ` +
                `Actions: ${directive.actions.join(', ')}`);
            throw new Error(`Custom actions are not allowed in ${framework} framework. ` +
        ...
        throw new Error(`Invalid actions type. Expected string (profile name) or array of strings, got: ${typeof directive.actions}`);
    }
    // Validate service prefix match
    ...
    try {
        ...
        const targetName = context.target && typeof context.target.getName === 'function'
            ? context.target.getName()
            : 'unknown';
        console.error(`[COMPLIANCE-AUDIT] Action(s) rejected by allow-list. ` +
            `Source: ${sourceName}, ` +
            `Target: ${targetName}, ` +
            `Capability: ${directive.capability || 'unknown'}, ` +
