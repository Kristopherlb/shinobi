 ...
 */
const allowListRegistry: Map<ComplianceFramework, ActionAllowListConfig> = new Map();
 ...
 */
function loadAllowListsFromConfig(framework: ComplianceFramework): ActionAllowListConfig {
  ...
  try {
    ...
    // Validate structure
    const validated: ActionAllowListConfig = {};
    for (const [servicePrefix, actions] of Object.entries(allowLists)) {
      if (Array.isArray(actions)) {
        validated[servicePrefix] = actions.filter((action: any) => typeof action === 'string');
      }
    }
 ...
 */
export function getActionAllowList(servicePrefix: string, framework: ComplianceFramework): string[] | undefined {
 ...
 */
export function isActionAllowed(action: string, servicePrefix: string, framework: ComplianceFramework): boolean {
  ...
  if (!allowList) {
    console.warn(
      `[SECURITY] No action allow-list defined for service '${servicePrefix}' in ${framework} framework. ` +
 ...
 */
export function validateActionsAgainstAllowList(
  actions: string[],
  ...
  if (disallowedActions.length > 0) {
    ...
    throw new Error(
      `Action(s) not allowed for service '${servicePrefix}' in ${framework} framework: ${disallowedActions.join(', ')}. ` +
 ...
 */
export function areCustomActionsAllowed(framework: ComplianceFramework): boolean {
 ...
 */
export function registerActionAllowList(
  servicePrefix: string,
  framework: ComplianceFramework,
  allowList: string[]
): void {
